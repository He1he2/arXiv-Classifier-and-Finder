...
def load_resources():
    ...
    def create_mlp():
        ...
        mlp.build(input_shape=(None, vectorizer.vocabulary_.__len__()))
        return mlp
    mlp = create_mlp()
...
def predict_categories(texts, vectorizer, mlb, model):
    ...
    texts_vec_coo = coo_matrix(texts_vec)
...
def format_prediction(prediction_tuple):
    combined = ''.join(prediction_tuple)
...
if st.button("运行分类与查询"):
    if not user_input.strip():
        ...
        with st.spinner("正在分类..."):
            ...
            format_cat = format_prediction(predicted_categories[0])
            category_code = ''.join(predicted_categories[0]).replace(' ', '').replace('.', '')
            category_name = category_map.get(category_code, format_cat)
            st.success("分类完成！")
            st.write(f"**预测类别**: {category_name}")
        ...
        with st.spinner("正在查询相关研究论文..."):
            ...
            for i in range(0, num_results):
                ...
                st.markdown(f"https://arxiv.org/pdf/{arxiv_id}({link_to_pdf})")
